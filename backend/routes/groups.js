...
router.get('/', function(req, res) {
    Group.find({}).sort({timestamp: -1}).populate('disturbances').exec(function(err, groups) {
        if(err){
...
router.get('/:id', function(req, res) {
    Group.findById(req.params.id).populate('disturbances').exec(function(err, group){
        if(err){
